...
def mkfs_xvdc_ext4():
  dev = ""
  #devs = ["xvdc", "xvdf"]
  devs = ["emulab-node0--bs"]
  for d in devs:
    if d in os.popen("ls /dev/mapper/%s" % d).read():
      dev = d
      break
  if dev == "":
    assert(False)
  all_run("umount /mnt; mkfs.ext4 /dev/mapper/%s; mount /dev/mapper/%s /mnt" % (dev, dev))
...
def run_dummy_task():
  ...
  output = run_subprocess( "/mydata/lexu/disaggregation/memeater/memeater")
  print(output)
